...
from typing import Any, Awaitable, Callable, Deque, Mapping, Sequence
...
class ActivityEngine:
    ...
    def add_listener(self, callback: Callable[[EngineSnapshot], Any]) -> Callable[[], None]:
    ...
    async def reconcile(self) -> EngineSnapshot:
        ...
        if any(f.classification is DriftClass.FATAL for f in findings) and not transitioning:
    ...
    async def _run_transition(self, activity_key: str) -> None:
        async with self._lock:
            ...
            if plan.is_empty:
                ...
                self._finish(activity_key, succeeded=True)
                return

            try:
                result = await self._executor.execute(
                    plan, target=target, on_progress=self._on_progress
                )
            except asyncio.CancelledError:
            ...
            if result.succeeded:
                self._finish(activity_key, succeeded=True, result=result)
            else:
    ...
    def _activity_or_raise(self, key: str):
        try:
