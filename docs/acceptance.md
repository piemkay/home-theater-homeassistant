...
| A33 | Turn the room's light down without starting the theater (0.8) | ✅ automated + **verified live** | `kino-card.test.js` "the light card" / "light capabilities", `test_schema.py` (8 cases), `kino-panel.test.js` "the light row editor"; live 2026-09-06: with Steam running, tapped Spots (lamp came on, row read 100%) and Dunkel (scene applied, curtain LED went down) |
| A35 | Configure the light card without naming a single entity (0.8.2) | partial | `test_schema.py` "an area fills the card…" (LightPanel.resolve: order, overrides, exclusions), `kino-panel.test.js` "the light row editor" (area round-trip, the hide toggle); **live**: pick Kino in the panel's Licht screen — the card must fill with that area's scenes and lights, and unticking one must remove it |
| A34 | Set the room from the sofa without knowing which lamp is which (0.8.1) | partial | `kino-card.test.js` "the light card": the active-scene rule, the fold, and per-lamp controls gated on `supported_color_modes`; **live**: apply a scene, then dim one lamp by hand — the header must fall back to "Manuell" |
| A36 | Start the evening from one screen: see what the room is doing, pick an activity, set the light and find a film without scrolling past anything that is not one of those (0.9) | ✅ automated + **verified in the harness** | `kino-card.test.js` "the Start screen (FR-78)" (16 cases: the always-open tile grid, the Ausschalten row and the states that must and must not offer it, the compact dropdown during a transition, the segmented control, the three poster rows, the transition body), "statusTitle (FR-78)", "device chips"; harness: off / running / mid-transition at 390×800 in both themes; **live**: the four Kino scenes and the activity tiles must fill the first screen with no header above them |
| A37 | Browse 753 titles from the sofa without the toolbar or the tabs ever scrolling away, and get back to the room in one tap (0.10) | ✅ automated + **verified in the harness** | `kino-card.test.js` "pushed views" (6 cases: what each source tile opens, that Aus opens nothing, the pinned source key, scroll-to-top, tabs replacing rather than stacking), "the screens under the segmented control speak one language" (the fixed tab set, the film-only toolbar, a bar on every pushed view and none on Home); harness: the bar measured at 0 px movement over a 385 px scroll, bar + scroller + footer = the frame exactly; **live**: ‹ Zurück from the library must land on Home with the room controls, and the filter sheet must still cover the bar |
