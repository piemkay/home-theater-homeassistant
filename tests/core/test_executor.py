...
async def test_cold_start_runs_devices_concurrently(config, drivers, clock):
    ...
    result = await clock.run(
        _executor(drivers, clock).execute(plan, target=config.activities["film"])
    )
...
async def test_settings_are_applied_after_the_device_is_ready(
...
async def test_dependencies_serialise_only_where_real(config_doc, clock):
    ...
    await clock.run(
        _executor(drivers, clock).execute(plan, target=config.activities["musik"])
    )
...
async def test_playback_is_stopped_before_its_player_powers_down(
    ...
    await clock.run(
        _executor(drivers, clock).execute(plan, target=config.activities["netflix"])
    )
...
async def test_unverified_shutdown_raises_but_does_not_block_others(
    ...
    result = await clock.run(
        _executor(drivers, clock).execute(plan, target=config.activities["aus"])
    )
...
async def test_progress_reports_a_named_bottleneck(config, drivers, clock):
    ...
    await clock.run(
        _executor(drivers, clock).execute(
            plan,
...
async def test_durations_are_learned_from_observed_runs(config, drivers, clock):
    ...
    await clock.run(
        _executor(drivers, clock, estimator).execute(
            plan, target=config.activities["musik"]
        )
    )
